...
var product = {
  ...
  renderProduct: function renderProduct() {
    ...
    $('#product').html(this.data.map(function (val, key) {
      return val.productList.map(function (item, key) {
        console.log(item);
        return ['<div class="product-item">', '<div class="item-content">', '<a href="./../index.html?productId=' + item.productId + '">', '<div class="item-img">', '<img src="' + appConfig.urlBase + item.productThumb + '" />', '<div class="img-label">', _this.renderLabel(item), '</div>', '</div>', '</a>', '<div class="item-detail">', '<div class="item-productName">' + item.productName + '</div>', '<div class="item-productBrief">' + item.productName + '</div>', _this.renderPrice(item), '</div>', '</div>', '</div>'].join('');
      }).join('');
    }).join(''));
  },
  isPromoteHandle: function isPromoteHandle(item) {
  ...
  renderPrice: function renderPrice(item) {
    var isPromote = this.isPromoteHandle(item);

    return ['<div class="item-price">', '<div class="price-left">' + (isPromote ? '优惠价格' : '套餐价格') + '</div>', '<div class="price-right">', '\uFFE5<span>' + (isPromote ? item.promotePrice : item.productPrice) + '</span> RMB', '</div>', '</div>'].join('');
  },
