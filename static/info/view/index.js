...
function init() {
	...
	$.ajax({
		type:"GET",
		url: appConfig.version + "/gather/link/"+localStorage.getItem('_uniqueKey')+"/getGatherInfo.do",
...
function renderMain() {
	...
	$.ajax({
		type:"GET",
		url: appConfig.version + "/admin/"+localDate.createBy+"/getAdminInfo.do",
		contentType:"application/json; charset=utf-8",
		success: function (val) {
			if (val.result == "0") {
				$(".qrCode").html(
					"<img src='" + appConfig.urlBase + val.data.qrCode + "'/>"
				);
