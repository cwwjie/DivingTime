$(document).ready(function() {
	...
	scrollTop();
	// 公告
	$("#colseNotice").click(function(){$("#Notice").css('display', 'none');});
});
function Logout() {
	$.ajax({
		type: "POST", 
		...
		success: function (message) {
			if (message.result == "0" ) {
				...
				$("#login").html(
					"<div><a href='./user/signup.html' target='_blank'>注册</a>"//
		...
		success: function (message) {
			if (message.result == "0" ) {
				...
				$("#login").html("<div class='Center'><i class='left'></i><span>"//
					...
					+"<a href='./user/account.html#Client' onclick='window.location.reload()'><li><i style='background-position:-376px -185px;'></i>旅客信息</li></a>"//
